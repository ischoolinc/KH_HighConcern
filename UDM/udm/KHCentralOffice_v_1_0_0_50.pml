<Project Name="KHCentralOffice">
	<Property Name="UDT">
		...
		<Release Version="1.0.0.47" URL="udm/UDT_1_0_0_47.tcmd" />
		<Release Version="1.0.0.48" URL="udm/UDT_1_0_0_48.tcmd" />
		<Release Version="1.0.0.49" URL="udm/UDT_1_0_0_49.tcmd" />
		<Release Version="1.0.0.50" URL="udm/UDT_1_0_0_50.tcmd"></Release>
	</Property>
	<Property Name="UDS">
...
where tag.prefix='聯課活動' and tag.name='社團' and @@Condition  @@Order]]>
	...
	<Pagination Allow="True" />
		...
		<Service Enabled="true" Name="GetSysAdminInfo">
			<Definition Type="javascript">
	<Code>
		<![CDATA[
		var request = getRequest().Request || getRequest();
...
WITH school_info AS( \
    SELECT \
        ...
        school_info.associated_with_name, \
        school_info.associated_with_cell_phone, \
        school_info.associated_with_email \
    FROM \
        ( \
            SELECT \
                array_to_string( \
                    xpath('//ChineseName/text()', xmlparse(content xmlEle)), \
                    '' \
                ) AS school_name, \
                array_to_string( \
                    xpath( \
                        '//AssociatedWithName/text()', \
                        xmlparse(content xmlEle) \
                    ), \
                    '' \
                ) AS associated_with_name, \
                array_to_string( \
                    xpath( \
                        '//AssociatedWithCellPhone/text()', \
                        xmlparse(content xmlEle) \
                    ), \
                    '' \
                ) AS associated_with_cell_phone, \
                array_to_string( \
                    xpath( \
                        '//AssociatedWithEmail/text()', \
                        xmlparse(content xmlEle) \
                    ), \
                    '' \
                ) AS associated_with_email \
            FROM \
                ( \
                    SELECT \
                        unnest( \
                            xpath('//SchoolInformation', xmlparse(content content)) \
                        ) AS xmlEle \
                    FROM \
                        list \
                    WHERE \
...
return { User: list };
	...
	<Resources />
		...
		<Service Enabled="true" Name="GetUserLoginInfo">
			<Definition Type="javascript">
	<Code>
		<![CDATA[
		var request = getRequest().Request || getRequest();
...
SELECT \
    school_info.school_name, \
    user_info.login_name AS user_name, \
    server_time, \
    client_info, \
    sys_admin, \
    description \
FROM \
    user_login \
    RIGHT JOIN user_info ON user_login.actor = user_info.login_name \
	CROSS JOIN school_info \
ORDER BY \
    school_name, \
    user_name  \
";
...
return { User: list };
	...
	<Package Name="data_exchange">
		<Service Enabled="true" Name="GetAbsenceRecordsByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
		throw '缺少StudentIDs';
		var StudentIDs = [].concat(request.StudentIDs.StudentID || []);

		var _sql ;
		var _return = [];
		_sql = getResource('mainSQL');
		_sql = _sql.replace(/@@StudentIDs/ig, StudentIDs.join(','));
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetDailyLifeByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetLearningServiceByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetMeritRecordsByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetSemsScoreByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetStudentExtInfoByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		<Resource Name="mainSQL">
			<![CDATA[
				SELECT 
					...
					identified_date,
					is_gifted,
					is_parent_handcapped,
					is_dependents_from_china,
					is_oversea_student,
					is_from_hk,
					is_frontier,
					is_foreign,
					is_exp_staff_child,
					is_physical_excellence,
					is_gov_emp_child,
					is_gov_emp_bereaved_official,
					is_gov_emp_bereaved_ill_accident,
					is_special_situation_family,
					income_type,
					approved_date,
					expired_date ,
					aboriginal_type,
		...
		<Service Enabled="true" Name="GetStudentIDBySchoolYearSemester">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		...
		var _return = [];
		_sql = getResource('mainSQL');
		_sql = _sql.replace(/@@SchoolYear/ig, request.SchoolYear);
		_sql = _sql.replace(/@@Semester/ig, request.Semester);
		_return = executeSql(_sql).toArray();
		return {data: _return};
	</Code>
	<Resources>
		<Resource Name="mainSQL">
			...
			WITH sems_history_data AS(
				SELECT history.id, 
					...
					array_to_string(xpath('//History/@Teacher', history_xml), '')::text as teacher 
				FROM (
		...
		<Service Enabled="true" Name="GetStudentsByIDs">
			<Definition Type="javascript">
	<Code>
		var request = getRequest().Request || getRequest();
		if (!request.StudentIDs)
	...
	<Resources>
		...
		<Service Enabled="true" Name="GetStudentTagKHName">
			<Definition Type="javascript">
	<Code>
		var _sql ;
		var _return = [];
	...
	<Resources>
		<Resource Name="mainSQL">
			<![CDATA[
	        		 ...
	        		 FROM tag
	            INNER JOIN tag_student
		            ON tag.id = tag_student.ref_tag_id 
                INNER JOIN student 
                    ON student.id = tag_student.ref_student_id 
		        WHERE 
			        prefix = '學籍資料上傳'
			]]>
		</Resource>
